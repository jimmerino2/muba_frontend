...
const { data, loading, error, refresh } = useAsync(async () => {
  const detail = await tpaApi.getClaimById(auth.orgId!, claimId)
...
const policyFacts = computed(() => {
  ...
  return [
    ...
    {
      label: 'Your delegated limit',
      value: p.tpaApprovalLimit !== null ? money(p.tpaApprovalLimit) : 'Not delegated',
    },
...
const decidable = computed(() => {
  ...
  if (claim.status !== 'pending_review' && claim.status !== 'verified') return false
  return policy.tpaApprovalLimit !== null && claim.amountRequested <= policy.tpaApprovalLimit
})
</script>
  ...
  <div>
    ...
    <template v-else-if="data">
      <PageHeader
        ...
        :back="{ to: '/tpa/claims', label: 'All claims' }"
      >
        <template #actions>
          <ClaimStatusBadge :status="data.claim.status" />
          <RouterLink v-if="decidable" :to="`/tpa/review/${data.claim.id}`" class="btn-primary">
            Decide this claim
          </RouterLink>
        </template>
      </PageHeader>

      <!-- Unverified claims are the TPA's own action: run the router. -->
      <div
      ...
      <div
        v-if="data.claim.status === 'pending_review' && !decidable"
        class="surface mb-5 border-l-2 border-l-amber-500 p-4"
      >
        <p class="text-sm font-medium text-mist-100">Escalated to the insurer</p>
        <p class="mt-0.5 text-sm text-mist-500">
          This claim exceeds your delegated approval limit and is decided by the insurer instead.
        </p>
      </div>
      ...
      <div class="grid gap-5 lg:grid-cols-[1.35fr_1fr]">
        ...
        <div class="space-y-5">
          ...
          <RouterLink
            v-if="data.payment"
            :to="`/tpa/payments/${data.payment.id}`"
