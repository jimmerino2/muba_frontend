...
/**
 * A TPA is neither payer nor payee, but the backend accepts either the payer
 * (the insurer) or the claim's administering TPA as the initiator — so this
 * reuses the same org-agnostic `paymentsApi` every other role's payment
 * detail screen calls; the bearer token is what the backend actually checks.
 */
  ...
  <div>
    ...
    <template v-else-if="data">
      <PageHeader
        ...
        :back="{ to: '/tpa/payments', label: 'All payments' }"
      >
        <template #actions>
          <RouterLink :to="`/tpa/claims/${data.payment.claimId}`" class="btn-ghost">
            View claim
          </RouterLink>
        </template>
      </PageHeader>
      ...
      <!-- Action prompt: TPA-initiated settlement, insurer-funded -->
      <div
