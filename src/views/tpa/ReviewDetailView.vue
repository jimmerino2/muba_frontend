...
const context = computed(() => {
  ...
  return [
    ...
    { label: 'Deductible', value: money(d.policy.deductible) },
    {
      ...
      value: d.policy.tpaApprovalLimit !== null ? money(d.policy.tpaApprovalLimit) : 'Not delegated',
    },
  ]
})
</script>
  ...
  <div>
    ...
    <template v-else-if="data">
      <PageHeader
        ...
        :back="{ to: '/tpa/review', label: 'Review queue' }"
      >
        <template #actions>
          <ClaimStatusBadge :status="data.claim.status" />
          <RouterLink :to="`/tpa/claims/${data.claim.id}`" class="btn-ghost">
            Full claim record
          </RouterLink>
        </template>
      </PageHeader>

      <div v-if="!withinLimit" class="surface mb-5 border-l-2 border-l-amber-500 p-4">
        <p class="text-sm font-medium text-mist-100">Above your delegated limit</p>
        <p class="mt-0.5 text-sm text-mist-500">
          This claim exceeds the approval limit the insurer has delegated to you and must be
          decided by the insurer instead.
        </p>
      </div>
