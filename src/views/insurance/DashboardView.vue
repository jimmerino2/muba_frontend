...
<template>
  <div>
    <PageHeader
      :title="auth.user?.orgName ?? 'Insurer dashboard'"
      subtitle="Your claim queue, exposure and settlement position — with the split between automated and human decisions in plain sight."
    >
      <template #actions>
        <RouterLink to="/insurance/review" class="btn-primary">Open review queue</RouterLink>
      </template>
    </PageHeader>
    ...
    <section class="mb-6">
      ...
      <div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard
          ...
          tone="attention"
          :hint="data ? `${moneyCompact(data.exposureAmount)} exposure` : undefined"
          :loading="loading"
          to="/insurance/payments"
        />
        <StatCard
          label="Settled"
    ...
    <section class="surface overflow-hidden">
      ...
      <EmptyState
        ...
        body="No claims are waiting on verification or an assessor decision."
        icon="✓"
      />
