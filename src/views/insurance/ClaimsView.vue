...
const { data, loading, error, refresh } = useAsync(() => insuranceApi.getClaims(auth.orgId!))

/** The insurer filters on the raw lifecycle — no re-labelling, since they own it. */
const status = ref('all')
const STATUS_OPTIONS = computed(() => [
  { value: 'all', label: 'All statuses' },
  // Drafts never reach the insurer, so they are not offered as a filter.
...
<template>
  <div>
    <PageHeader
      title="Claims"
      subtitle="Every claim submitted against your policies. Drafts held by providers are not visible here."
    />
    ...
    <DataTable
      ...
      :row-key="(row) => row.id"
      :loading="loading"
      :to="(row) => `/insurance/claims/${row.id}`"
      searchable
