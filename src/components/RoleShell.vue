...
const NAV: Record<Role, NavItem[]> = {
  ...
  insurance: [
    ...
    { to: "/insurance/policies", label: "Policies (Customers)", glyph: "⛨", hint: "Policyholder cover, limits and thresholds" },
    { to: "/insurance/review", label: "Escalated Queue", glyph: "⚖", hint: "Claims above the TPA’s delegated limit" },
    { to: "/insurance/payments", label: "Payments", glyph: "⇄", hint: "Settlements you have triggered" },
  ],
  tpa: [
    { to: "/tpa/dashboard", label: "Dashboard", glyph: "◈", hint: "Delegated queue and decision overview" },
    { to: "/tpa/claims", label: "Claims", glyph: "◇", hint: "Every claim you administer" },
    { to: "/tpa/review", label: "Review queue", glyph: "⚖", hint: "Claims within your delegated limit" },
    { to: "/tpa/payments", label: "Payments", glyph: "⇄", hint: "Settlements you have triggered" },
  ],
};
...
const ROLE_TITLE = computed(() => ROLE_TITLES[props.role]);

const contextLine = computed(() => (auth.user?.orgName ? auth.user.orgName : "Personal cover"));
...
<template>
  <div class="flex h-full">
    <!-- ------------------------------------------------------------ sidebar -->
    <aside
      class="fixed inset-y-0 left-0 z-40 flex w-64 shrink-0 flex-col border-r border-ink-800 bg-ink-900/90 backdrop-blur-md transition-transform lg:static lg:translate-x-0"
    ...
    >
      ...
      <nav class="flex-1 space-y-0.5 overflow-y-auto px-3" aria-label="Primary">
        <RouterLink
          ...
          :class="isActive(item.to) ? 'bg-ink-800 text-mist-100' : 'text-mist-400 hover:bg-ink-850 hover:text-mist-200'"
    ...
    <div v-if="mobileNavOpen" class="fixed inset-0 z-30 bg-ink-950/70 lg:hidden" @click="mobileNavOpen = false" />

    <!-- --------------------------------------------------------------- main -->
    <div class="flex min-w-0 flex-1 flex-col">
      <header
        class="sticky top-0 z-20 flex items-center gap-3 border-b border-ink-800 bg-ink-900/85 px-4 py-3 backdrop-blur-md sm:px-6"
      >
        <button
          type="button"
